...
├── admin/app.py              # Gradio admin panel (8 tabs: Users, Sessions, Costs, Analytics, Alerts, System, Broadcast, Whitelist)
...
9 tables, SQLAlchemy 2.0 async ORM. `users.telegram_id` (BIGINT) as PK.
...
9 tables in `db/models.py`. PK for `users` is `telegram_id` (BIGINT). All FKs use `ondelete="CASCADE"` or `ondelete="SET NULL"`.
